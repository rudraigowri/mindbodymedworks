  ...
  return (
    <section id="contact" aria-labelledby="contact-heading" className="bg-background">
      <div className="cta-shell mx-auto grid max-w-6xl items-center gap-16 rounded-[2rem] px-6 py-28 lg:grid-cols-[1.1fr_0.9fr] lg:px-10 lg:py-36">
        <div>
          <h2 id="contact-heading" className="font-serif section-title text-deep-teal">
          ...
          <div className="mt-12">
            <WhatsAppButton showIcon={false}>Get started today</WhatsAppButton>
          </div>
        </div>
        <ul className="grid grid-cols-3 gap-4 text-center">
          {site.ctaSupports.map((item) => (
            <li key={item.title} className="support-card flex flex-col items-center gap-3 rounded-[1.4rem] px-3 py-5">
              ...
              <p className="text-sm font-medium leading-snug text-deep-teal">{item.title}</p>
            </li>
          ))}
        </ul>
      </div>
